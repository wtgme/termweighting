...
public class String2Number {
	...
	private int index = 1;
	private Map<String, Integer> labelSize = new HashMap<String, Integer>();
	private Map<String, Integer> finalLableSize = new HashMap<String, Integer>();
	...
	public void readVocTime(String trainFilePath) {
		...
		while ((line = fin.readLine()) != null) {
			...
			Integer size = this.labelSize.get(tokens[0]);
			if (size == null) {
				size = 0;
			}
			this.labelSize.put(tokens[0], size + 1);
		}
		fin.close();
	}
	...
	public void transform(String filePath, String outputPath) {
		...
		while ((line = fin.readLine()) != null) {
			...
			// && this.labelSize.get(label) > 20
				...
				Integer finalsize = this.finalLableSize.get(label);
				if (finalsize == null) {
					finalsize = 0;
				}
				this.finalLableSize.put(label, finalsize + 1);
	...
	public void outputLabel(String filePath) {
		...
		for (Iterator<String> ite = keySet.iterator(); ite.hasNext();) {
			...
			Integer size = this.finalLableSize.get(key);
			fout.append(key + "\t" + value + "\t" + size + "\n");
		}
		fout.flush();
		fout.close();
	}
	public int run(String dataset)
