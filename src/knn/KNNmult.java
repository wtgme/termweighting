...
public class KNNmult {
	...
	public String prediction(String document) {
		...
		for (Iterator<Integer> iteId = docIDs.iterator(); iteId.hasNext();) {
			...
			// Double confidence =
			// this.documentConfidence.get(documentID.get(i));
	...
	public void runClassifier(String filePath, String i, String thread) {
		...
		for (File file : files) {
			try {
				...
				runKNN(filePath, filePath + "/knndata/" + trainFile
						...
						thread, filePath + "/knndata/" + testFile
								+ weightFilePath, filePath + "/prediction/" + i
								+ weightFilePath);
				index = 0;
	...
	public void runClassifier(String filePath, String i, String thread,
		...
		for (String weightFilePath : fileNames) {
			try {
				...
				intialKNN();
				runKNN(filePath, filePath + "/knndata/" + trainFile
						+ weightFilePath + ".txt", filePath + "/knndata/"
						+ confiFile, thread, filePath + "/knndata/" + testFile
						+ weightFilePath + ".txt", filePath + "/prediction/"
						+ i + weightFilePath);
				index = 0;
				System.out.println((System.currentTimeMillis() - begin) / 1000);
			} catch (Exception e) {
