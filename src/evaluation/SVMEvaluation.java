...
public class SVMEvaluation {
	...
	public void readTrueLabel(String filePath) {
		System.out.println("True Label:" + filePath);
		...
		System.out.println("Prediction:" + trueLabel.size());
		fin.close();
	}

	public void readPrediction(String filePath) {
		System.out.println("Prediction:" + filePath);
		FileReaderInterface fin = new FileReaderInterface();
		...
		System.out.println("Prediction:" + predictionLabel.size());
		fin.close();
	}
	...
	public void outPut(String filePath, String foldK) {
		FileWriterInterface fout = new FileWriterInterface();
		fout.writeFile(filePath + "/svm/evaluation/" + foldK + "evaluation.txt");
	...
	public void runEvaluation(String filePath, String i) {
		...
		for (String scheme : lists) {
			...
			readTrueLabel(filePath + "/svm/data/" + i + "test" + scheme
					+ ".txt");

			readPrediction(filePath + "/svm/prediction/" + i + scheme + ".txt");
			evaluation(scheme);
		}
		// for (File file : files) {
	...
	public static void main(String[] args) {
		...
		for (int i : sizes) {
			...
			for (String scheme : lists) {
				...
				evaluation.readTrueLabel(filePath + "/svm/data/" + i + "test"
						+ scheme + ".txt");

				evaluation.readPrediction(filePath + "/svm/prediction/" + i
						+ scheme + ".txt");
				evaluation.evaluation(scheme);
			}
			evaluation.outPut(filePath, "" + i);
		}
	}
}
